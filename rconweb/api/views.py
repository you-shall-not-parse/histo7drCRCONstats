...
def is_history_only() -> bool:
    return os.getenv("HISTORY_ONLY", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def get_archive_public_info() -> PublicInfoType:
    ...
    with enter_session() as sess:
        latest_map = (
            sess.query(Maps)
            .filter(Maps.server_number == get_server_number())
            .order_by(Maps.start.desc())
            .first()
        )

        if latest_map is None:
            latest_map = sess.query(Maps).order_by(Maps.start.desc()).first()
...
def get_public_info(request):
    if is_history_only():
        return api_response(
            result=get_archive_public_info(),
            failed=False,
